...
```{r}
# Set the file path (replace 'file_path' with your actual file path)
file_path <- "walmart_data.csv"

# Load the Walmart data
walmart_data <- read_csv(file_path, show_col_types = FALSE)

# Display a list of columns to choose two variables for the analysis
names(walmart_data)
```
...
cat("\n----------------------------------------\n")
cat("# 1. Hypotheses for Walmart_Fair_Prices Results:\n")
cat("\n----------------------------------------\n")
# 1. Hypotheses for Walmart_Fair_Prices
#Null Hypothesis (H0): Average score for Walmart's fair prices is 5.
#Alternative Hypothesis (H1): Average score for Walmart's fair prices is not 5.
# - State the critical value of t-test using the normal (z) distribution. Use the qnorm() function. 
alpha <- 0.05
critical_value_z <- qnorm(1 - alpha/2)  # Two-tailed test
critical_value_z

# - State the value of the test statistic. 
population_mean_fair_prices <- 5
fair_prices_data <- walmart_data$Walmart_Fair_Prices
t_statistic_fair_prices <- t.test(fair_prices_data, mu = population_mean_fair_prices)$statistic
t_statistic_fair_prices
# - Compare the test statistic to the critical value.
compare_to_critical_value_fair_prices <- t_statistic_fair_prices > critical_value_z
compare_to_critical_value_fair_prices
# - State the p-value of the test. 
p_value_fair_prices <- t.test(fair_prices_data, mu = population_mean_fair_prices)$p.value
p_value_fair_prices
# - State the conclusion of the test.
cat("\n----------------------------------------\n")
cat("Hypotheses for Walmart_Returns_Policy Results:\n")
cat("\n----------------------------------------\n")

# 1. Hypotheses for Walmart_Fair_Prices
#Null Hypothesis (H0): Average score for Walmart's returns policy is 4.
#Alternative Hypothesis (H1): Average score for Walmart's returns policy is not 4.
# - State the critical value of t-test using the normal (z) distribution. Use the qnorm() function. 
alpha <- 0.05
critical_value_z <- qnorm(1 - alpha/2)  # Two-tailed test
critical_value_z
...
# - State the value of the test statistic. 
population_mean_returns_policy <- 4
returns_policy_data <- walmart_data$Walmart_Returns_Policy
t_statistic_returns_policy <- t.test(returns_policy_data, mu = population_mean_returns_policy)$statistic
t_statistic_returns_policy
# - Compare the test statistic to the critical value.
compare_to_critical_value_returns_policy <- t_statistic_returns_policy > critical_value_z
compare_to_critical_value_returns_policy
# - State the p-value of the test. 
p_value_returns_policy <- t.test(returns_policy_data, mu = population_mean_returns_policy)$p.value
p_value_returns_policy 
# - State the conclusion of the test.

# In up to 300 words, describe a situation or problem from your job, everyday life, current events, etc., for which a one-sample t-test would be useful for decision making. Describe the business problem and stakeholders. i.e. Who (person, group, organization) would be interested in solving this business problem? Describe up to 5 variables that can be used.

#Solution: Is in the report.

```
```{r}
# Step 1: Loads the data set
walmart_data <- read.csv("walmart_data.csv")

# Step 2: Compute the Mean
mean_fair_prices <- mean(walmart_data$Walmart_Fair_Prices)
mean_returns_policy <- mean(walmart_data$Walmart_Returns_Policy)
mean_fair_prices
mean_returns_policy 
# Step 3: Conduct the One-Sample T-Test
t_test_fair_prices <- t.test(walmart_data$Walmart_Fair_Prices, mu = mean_fair_prices)
t_test_returns_policy <- t.test(walmart_data$Walmart_Returns_Policy, mu = mean_returns_policy)
t_test_fair_prices
t_test_returns_policy
# Step 4: Determine the Critical Value
# Assuming a 95% confidence level for a two-tailed test
critical_value_95 <- qnorm(0.975)
critical_value_95
# Step 5: Compare and Conclude
# This part is usually interpretative based on the output from the t-test and the critical value
...
```{r}
hist(walmart_data$Walmart_Quality, 
     main="Histogram of Walmart_Quality", 
     xlab="Quality Score", 
     ylab="Frequency", 
     col="blue", 
     border="black")
...
hist(walmart_data$Walmart_Variety, 
     main="Histogram of Walmart_Variety", 
     xlab="Variety Score", 
     ylab="Frequency", 
     col="green", 
     border="black")
...
# Histogram for Walmart_Quality
p1 <- ggplot(walmart_data, aes(x = Walmart_Quality)) +
      geom_histogram(bins = 10, fill = "blue", color = "black") +
      ggtitle("Histogram of Walmart_Quality") +
      xlab("Quality Score") +
      ylab("Frequency")

# Histogram for Walmart_Variety
p2 <- ggplot(walmart_data, aes(x = Walmart_Variety)) +
      geom_histogram(bins = 10, fill = "green", color = "black") +
      ggtitle("Histogram of Walmart_Variety") +
      xlab("Variety Score") +
      ylab("Frequency")
...
# Boxplot for Walmart_Quality
p1 <- ggplot(walmart_data, aes(y = Walmart_Quality, x = factor(0))) +
      geom_boxplot(fill = "blue") +
      labs(title = "Boxplot of Walmart_Quality", x = "", y = "Quality Score")

# Boxplot for Walmart_Variety
p2 <- ggplot(walmart_data, aes(y = Walmart_Variety, x = factor(0))) +
      geom_boxplot(fill = "green") +
      labs(title = "Boxplot of Walmart_Variety", x = "", y = "Variety Score")
